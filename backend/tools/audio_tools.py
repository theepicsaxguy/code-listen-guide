    ...
    try:
        ...
        if not settings.openai_api_key:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="cba_tts_", suffix=".mp3", delete=False
            ) as temp_file:
        ...
        logger.info(
            f"Generated TTS audio: {temp_path} (voice={voice}, text_length={len(text)})"
        )
        return temp_path
        ...
        with tempfile.NamedTemporaryFile(
            prefix="cba_tts_error_", suffix=".mp3", delete=False
        ) as temp_file:
...
def concat_audio_with_chapters(
    chapter_paths: List[str], chapter_titles: List[str]
) -> str:
    """
    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(
            prefix="cba_audiobook_", suffix=".mp3", delete=False
        ) as temp_file:
        ...
        logger.info(
            f"Created audiobook: {output_path} (duration={len(combined)}ms, chapters={len(chapter_paths)})"
        )
        return output_path
        ...
        # Return placeholder
        with tempfile.NamedTemporaryFile(
            prefix="cba_concat_error_", suffix=".mp3", delete=False
        ) as temp_file:
            payload = "\n".join(
                f"{title}:{path}" for title, path in zip(chapter_titles, chapter_paths)
            )
