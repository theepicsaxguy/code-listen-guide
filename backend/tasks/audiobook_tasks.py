...
async def _start_audiobook_workflow(job_id: str, repo_url: str, depth_tier: str) -> None:
    with tracer.start_as_current_span("start_audiobook_workflow", attributes={"job_id": job_id}):
        workflow = _create_workflow(job_id=job_id, repo_url=repo_url, depth_tier=depth_tier)
...
async def _resume_audiobook_workflow(job_id: str) -> None:
    with tracer.start_as_current_span("resume_audiobook_workflow", attributes={"job_id": job_id}):
        ...
        if job is None:
            return
        workflow = _create_workflow(job_id=job_id, repo_url=job.repo_url, depth_tier=job.depth_tier)
