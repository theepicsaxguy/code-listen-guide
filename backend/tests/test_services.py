...
class TestRepositoryAnalyzer:
    ...
    async def test_analyzer_max_size_validation(self):
        ...
        analyzer = RepositoryAnalyzer(
            repo_url="https://github.com/user/huge-repo", max_repo_size_mb=100
        )
...
class TestPostProcessor:
    ...
    async def test_merge_chapter_audio_files(self, tmp_path):
        ...
        audio_files = [str(tmp_path / "chapter1.mp3"), str(tmp_path / "chapter2.mp3")]
    ...
    async def test_create_deliverables(self):
        ...
        job_data = {"id": "test-job", "repo_name": "test-repo"}
        ...
        result = await create_deliverables(job_data=job_data, chapters=chapters)
...
class TestStorageService:
    ...
    async def test_upload_audio_file(self, mock_s3_client, tmp_path):
        ...
        with patch(
            "backend.services.storage.boto3.client", return_value=mock_s3_client
        ):
            result = await upload_audio_file(
                job_id="test-job", chapter_number=1, file_path=str(audio_file)
            )
...
class TestPaymentService:
    ...
    async def test_create_payment_intent(self, mock_stripe_client):
        ...
        with patch("backend.services.payment.stripe", mock_stripe_client):
            result = await create_payment_intent(
                job_id="test-job", amount_cents=4900, user_email="test@example.com"
            )
    ...
    async def test_handle_payment_success_webhook(
        self, mock_stripe_client, test_db, create_job
    ):
        ...
        webhook_payload = {
            ...
            "data": {
                "object": {"id": "pi_test_123", "metadata": {"job_id": str(job.id)}}
            },
        }
    ...
    async def test_refund_payment(self, mock_stripe_client):
        ...
        with patch("backend.services.payment.stripe", mock_stripe_client):
            result = await process_refund(
                payment_intent_id="pi_test_123", reason="Job failed"
            )
...
class TestServiceIntegration:
    ...
    async def test_full_audiobook_generation_pipeline(
        self, sample_analysis_result, test_db, create_job
    ):
