...
export function DepthSelector({ selectedDepth, onDepthChange }: DepthSelectorProps) {
  return (
    <div className="grid md:grid-cols-3 gap-6 w-full">
      {depthTiers.map((tier) => (
        <Card key={tier.id} className={`relative hover-scale cursor-pointer transition-all ${tier.popular ? 'border-primary shadow-glow' : ''} ${selectedDepth === tier.id ? 'ring-2 ring-primary' : ''}`} onClick={() => onDepthChange(tier.id)}>
